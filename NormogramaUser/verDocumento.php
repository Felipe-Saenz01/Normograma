...
if(isset($_GET['visita'])){
    $sentenciaVista = $db->prepare("SELECT visitas FROM normativa WHERE codigo_gen = :codigo_gen;");
    $sentenciaVista->bindParam(':codigo_gen',$codigo, PDO::PARAM_INT);
    $sentenciaVista->execute();
    $vistaNorma = $sentenciaVista->fetch(PDO::FETCH_ASSOC);
    
    $nuevaVisita = $vistaNorma['visitas']+1;

    $SentenciaNuevaVisita = $db->prepare("UPDATE normativa SET visitas = :nuevaVisita WHERE codigo_gen= :codigo_gen;");
    $SentenciaNuevaVisita->bindParam(':nuevaVisita',$nuevaVisita, PDO::PARAM_INT);
    $SentenciaNuevaVisita->bindParam(':codigo_gen',$codigo, PDO::PARAM_STR);
    $resu = $SentenciaNuevaVisita->execute();

    if($resu){
	$db=null;
	header('location: verDocumento.php?id='.$codigo);
    }else{
	print "error";
    }
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/5.0.1/js/bootstrap.min.js"></script>
    ...
    <iframe class="embed-responsive-item" src="../Archivos/<?php echo $codigo."/".$resultado->directorio; ?>" width="100%" height="100$" allow="autoplay"></iframe>
    </div>
