<?php
    ...
    $sentencia = $db->query("SELECT * FROM dependencia_normativa ORDER BY nombre_dependencia ASC");
	...
	$busqedaDependencia = $db->prepare("SELECT * FROM dependencia_normativa WHERE codigo = ? ORDER BY nombre_dependencia ASC;");
	$busqedaDependencia->execute([$id]);
	$editDependencia = $busqedaDependencia->fetch(PDO::FETCH_OBJ);
    }
...
<?php
	...
	if($alertaGu==2){
		...
		Swal.fire({
			title: 'Guardado',
			text: "La dependencia se ha guardado correctamte",
			icon: 'success',
...
<?php
	...
	if($alertaDel==2){
		...
		Swal.fire({
			title: 'Eliminado',
			text: "Se ha eliminado la dependencia",
			icon: 'success',
			confirmButtonColor: '#037207',
			confirmButtonText: 'aceptar'
		}).then((result) => {
...
<?php
	...
	if($alertaActu==2){
		...
		Swal.fire({
			title: 'Actualizado',
			text: "Se ha actualizado la clasificacion",
			icon: 'success',
			confirmButtonColor: '#037207',
			confirmButtonText: 'aceptar'
		}).then((result) => {
...
<?php
	...
	if($alerError==2){
		...
		Swal.fire({
			...
			text: "No es posible eliminar la depencia porque esta siendo usada por una normativa",
			icon: 'warning',
			confirmButtonColor: '#037207',
			confirmButtonText: 'aceptar'
		}).then((result) => {
...
<?php
	...
	if($alertErrorDuplicado==2){
		...
		Swal.fire({
			...
			text: "No es posible guardar esta dependencia porque ya existe una dependencia con ese nombre.",
			icon: 'warning',
			confirmButtonColor: '#037207',
			confirmButtonText: 'aceptar'
		}).then((result) => {
...
<?php
	...
	if($alertErrorDefault==2){
		...
		Swal.fire({
			title: 'Error al Guardar!',
			text: "Ocurrió algún error al intentar guardar/actualizar la dependencia.",
			icon: 'warning',
			confirmButtonColor: '#037207',
			confirmButtonText: 'aceptar'
		}).then((result) => {
...
<div class="container-fluid" >
	...
	<h3>Lista Dependencias<button style="background-color: white;" type="button" class="btn btn-light" 
		data-bs-toggle="tooltip" data-bs-placement="top" 
		title="La tabla contiene la lista las diferestes dependencias que puede poseer una norma."><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-question-circle" viewBox="0 0 16 16">
	...
	<table class="table">
	    ...
	    <td style="text-align: center;">Nombre Dependencia</td>
	...
	<tr>
	    ...
	    <td style="text-align: center;"><a class="btn btn-warning" href="Dependencia.php?id=<?php echo $datos->codigo; ?>"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
		<path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
		<path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
		</svg></a></td>
	    <td style="text-align: center;"><a class="btn btn-danger" onClick= "alertDep(<?php echo $datos->codigo; ?>)"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
...
<div class="col-md-3 shadow h-110 d-inline-block" style="Height: 230px">
        <hr><h4><?php if($condicion==1){echo "Nueva Dependencia"; }else{echo "Editar Dependencia";}?></h4><hr>
	...
	<input class="form-control form-control-sm" type="text" name="nombre_dependencia" placeholder="Ingrese la dependencia que decea incluir." value="<?php if($condicion==2){echo $editDependencia->nombre_dependencia;} ?>" required>
    </div>

    <div class="d-grid gap-2 col-6 mx-auto">
	<input type="hidden" name="ocultoDependencia" value="1">
